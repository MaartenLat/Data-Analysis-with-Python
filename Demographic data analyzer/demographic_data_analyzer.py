...
def calculate_demographic_data(print_data=True):
    ...
    percentage = df.loc[df['salary']=='>50K','native-country'].value_counts()/df['native-country'].value_counts()
    highest_earning_country = percentage.index[percentage==percentage.max()].tolist()[0]
    highest_earning_country_percentage = round(percentage.max()*100,1)

    # Identify the most popular occupation for those who earn >50K in India.
    occupation_India = df.loc[(df['native-country']=='India') & (df['salary']=='>50K'),'occupation'].value_counts()
    top_IN_occupation = occupation_India.index[occupation_India==occupation_India.max()].tolist()[0]
